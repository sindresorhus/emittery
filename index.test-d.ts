...
// Strict typing for listeners
{
  ...
  const test = async () => {
    ...
    const event2 = await ee.once(["value", "numValue"]);
    expectType<string | number>(event2);
  };
...
// Strict typing for `.events` iterator
{
  const testEventsIterator = async () => {
    ...
    for await (const event of ee.events(["value", "numValue"])) {
